...
from .constants import PLAY_HISTORY_FILE
...
#######################################################################################################################
    ...
    async def cmd_back(self, player, channel, author, message, permissions, voice_channel):
        """
        Usage:
            {command_prefix}back

        Returns to the previous song when enough votes are cast.
        """
        if player.is_stopped:
            raise exceptions.CommandError("Can't return to the previous song! The player is not playing!", expire_in=20)

        with open(PLAY_HISTORY_FILE) as history_file:
            hist = history_file.readlines()
            if len(hist) < 1:
                raise exceptions.CommandError("There is no play history to return to!", expire_in=20)
            last_url = hist[len(hist)-1]

        # Voting
        num_voice = sum(1 for m in voice_channel.voice_members if not (
            m.id in [self.config.owner_id, self.user.id]
        ))
        num_votes = player.skip_state.add_skipper(author.id, message)
        ...
        if votes_remaining  <= 0:
            self.cmd_play(self, player, channel, author, permissions, None, back_url)
    ...
    async def cmd_addnp(self, player, channel, server, message):
        ...
        if self.autoplaylist.count(player._current_entry.url) == 0:
            ...
            await self.send_message(channel, "There are now **%s** songs in the playlist. ARE YOU HAPPY NOWWWWWWW?" % (len(player.autoplaylist)))
        else:
    ...
    async def cmd_removenp(self, player, channel, server, message):
        """
        Usage:
            {command_prefix}removenp
        Removes the currently playing song from the autoplaylist
        """
        if self.autoplaylist.count(player._current_entry.url) == 0:
            # Doesn't exist in the playlist, can't remove it
            await self.send_message(channel, "**%s** isn't in the autoplaylist!" %(player._current_entry.title))
        else:
            # Remove from memory, then from the config file
            temp_playlist_file = "config/_autoplaylist.txt"
            with open(self.config.auto_playlist_file) as fin, open(temp_playlist_file, "w") as fout:
                self.autoplaylist.remove(player._current_entry.url)
                for link in fin:
    ...
    async def cmd_gethistory(self, player, channel, server, limit=10):
        """
        Usage:
            {command_prefix}gethistory
            {command_prefix}gethistory n
